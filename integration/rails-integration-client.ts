...
export async function createFactory<T>(
  name: string,
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to create '${name}' factory: ${response.status} ${await response.text()}`
    )
  }
...
export async function fetchLoginToken(email: string): Promise<string> {
  const response = await fetch(
    `${BASE_URL}/tauri_integration/users/${encodeURIComponent(email)}/login_token`
  )
  if (!response.ok) {
    throw new Error(
      `Failed to fetch login token for '${email}': ${response.status} ${await response.text()}`
    )
  }
